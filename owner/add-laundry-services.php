...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_laundry'])) {
    ...
    // Get service options
    $pickup = ($_POST['pickup'] === 'Yes') ? 'Yes' : 'No';
    $delivery = ($_POST['delivery'] === 'Yes') ? 'Yes' : 'No';
    $dry_cleaning = ($_POST['dry_cleaning'] === 'Yes') ? 'Yes' : 'No';
    $washing = ($_POST['washing'] === 'Yes') ? 'Yes' : 'No';
    $ironing = ($_POST['ironing'] === 'Yes') ? 'Yes' : 'No';

    // ✅ Validate Phone Number
    if (!preg_match('/^[0-9]{10}$/', $contact_number)) {
        $_SESSION['error'] = "Invalid contact number format!";
        header("Location: add-laundry-services.php");
        exit();
    }

    // ✅ Check if Address Exists in `addresses` Table
    $address_query = "SELECT address_id FROM addresses WHERE LOWER(city) = LOWER('$city') AND LOWER(village) = LOWER('$village') AND LOWER(po) = LOWER('$po') AND LOWER(tehsil) = LOWER('$tehsil') AND LOWER(district) = LOWER('$district') AND LOWER(state) = LOWER('$state') AND pincode = '$pincode'";
    ...
    // ✅ Insert Data into `laundry_services`
    $query = "INSERT INTO `laundry_services`(`user_id`, `address_id`, `name`, `main_image`, `description`, `contact_number`, 
              `pickup`, `delivery`, `dry_cleaning`, `washing`, `ironing`, `opening_hours`, `closing_hours`)
              VALUES ('$user_id', '$address_id', '$name', '$main_image_name', '$description', '$contact_number',
              '$pickup', '$delivery', '$dry_cleaning', '$washing', '$ironing', '$opening_hours', '$closing_hours')";

    if (!mysqli_query($conn, $query)) {
        $_SESSION['error'] = "Error adding Laundry Service: " . mysqli_error($conn);
        header("Location: ./add-laundry-services.php");
        exit();
    }
...
// ✅ Insert Amenities into `laundry_service_amenities`
if (!empty($_POST['amenities'])) {
    $amenities_list = explode(",", $_POST['amenities']);
    
    // Debug output
    error_log("Amenities list: " . print_r($amenities_list, true));
    
    foreach ($amenities_list as $amenity_id) {
        $amenity_id = intval($amenity_id);
        if ($amenity_id > 0) {
            $amenity_query = "INSERT INTO laundry_service_amenities (laundry_service_id, amenity_id) VALUES ('$service_id', '$amenity_id')";
            
            // Execute query and check for errors
            if (!mysqli_query($conn, $amenity_query)) {
                error_log("Error inserting amenity $amenity_id: " . mysqli_error($conn));
            } else {
                error_log("Successfully inserted amenity $amenity_id");
            }
        }
    }
}
    ...
    $_SESSION['success'] = "Laundry service added successfully!";
...
<head>
    ...
    <title>Laundry Service Registration</title>
...
<body>
    ...
    <section class="form-section w-100">
        <p class="form-heading">Laundry Service Registration</p>
        <form action="./add-laundry-services.php" class="form-container" method="POST" enctype="multipart/form-data" class="form">
            <div>
            ...
            <div class="form-group">
                ...
                <div>
                    ...
                    <input type="tel" name="contact_number" pattern="[0-9]{10}" maxlength="10" 
            ...
            <div class="form-group">
                <div>
                    <label class="custum-file-upload" for="imageUpload">
                        ...
                        <div class="text">
                            <span>Click to upload additional service images</span>
                        </div>
            ...
            <button type="submit" name="add_laundry" class="custom-button">Add Laundry Service</button>
            <br>
            <br>
        </form>
    </section>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateHiddenInput() {
                    ...
                    $("#amenitiesHidden").val(idsString);
                    console.log("Selected amenities:", idsString); // Debug output
}
