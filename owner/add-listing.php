...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($address_result) > 0) {
        ...
        // Get Latitude & Longitude (Mock API Call or Manual Input)
    ...
    $query = "INSERT INTO accommodation_services (user_id, property_name, property_type, address_id, contact_number, description, main_image)
              VALUES ('$user_id', '$property_name', '$property_type', '$address_id', '$contact_number', '$description', '$main_image_name')";
    if (!mysqli_query($conn, $query)) {
        $_SESSION['error'] = "Error adding listing: " . mysqli_error($conn);
        header("Location: add-listing.php");
        exit();
    }
    ...
    if (!empty($_POST['amenities'])) {
        ...
        foreach ($amenities_list as $amenity_id) {
            ...
            if ($amenity_id > 0) {
                $amenity_query = "INSERT INTO accommodation_amenities (service_id, amenity_id) VALUES ('$service_id', '$amenity_id')";
...
<body>
    ...
    <section class="form-section w-100">
        <p class="form-heading">Property Registration</p>
        <form action="./add-listing.php" class="form-container" method="POST" enctype="multipart/form-data" class="form">
            <div>
            ...
            <button type="submit" class="custom-button">Submit Property</button>
            <br>
            <br>
        </form>
    </section>
