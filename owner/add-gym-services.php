...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_gym'])) {
    ...
    if (!preg_match('/^[1-9][0-9]{9}$/', $contact_number)) {
        ...
        header("Location: add-gym-services.php");
        exit();
    }
    ...
    $query = "INSERT INTO `gym_services`(`user_id`, `name`, `address_id`, `contact_number`, `main_image`, `description`, `opening_hours`, `closing_hours`)
              VALUES ('$user_id', '$name', '$address_id', '$contact_number', '$main_image_name', '$description', '$opening_hours', '$closing_hours')";
    if (!mysqli_query($conn, $query)) {
        $_SESSION['error'] = "Error adding Gym: " . mysqli_error($conn);
        header("Location: ./add-gym-services.php");
        exit();
    }
    ...
    if (!empty($_POST['amenities'])) {
        ...
        foreach ($amenities_list as $amenity_id) {
            ...
            if ($amenity_id > 0) {
                $amenity_query = "INSERT INTO gym_services_amenities (gym_service_id, amenity_id) VALUES ('$service_id', '$amenity_id')";
...
<body>
    ...
    <section class="form-section w-100">
        <p class="form-heading">Gym Registration</p>
        <form action="./add-gym-services.php" class="form-container" method="POST" enctype="multipart/form-data" class="form">
            <div>
            ...
            <div class="form-group">
                <div>
                    <label class="custum-file-upload" for="imageUpload">
                        ...
                        <div class="text">
                            <span>Click to upload extra Gym Images</span>
                        </div>
            ...
            <button type="submit" name="add_gym" class="custom-button">Add Gym</button>
            <br>
            <br>
        </form>
    </section>
